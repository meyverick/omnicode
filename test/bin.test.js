...
describe("omnicode.js CLI integration", () => {
  it("fails when a required tool is missing", () => {
    ...
    assert.throws(
      () => execFileSync(process.execPath, [binPath], { env, encoding: "utf8", timeout: 5000 }),
      /missing required tool\(s\): (opencode|omniroute)/
    );
  });

  it("shows usage for --help", () => {
    const output = execFileSync(process.execPath, [binPath, "--help"], { encoding: "utf8", timeout: 5000 });
    assert.ok(output.includes("Usage: omnicode"));
  });

  it("shows package version for --version", () => {
    const output = execFileSync(process.execPath, [binPath, "--version"], { encoding: "utf8", timeout: 5000 });
    assert.equal(output.trim(), getVersion());
  });

  it("shows process state for --status", () => {
    const output = execFileSync(process.execPath, [binPath, "--status"], { encoding: "utf8", timeout: 5000 });
  ...
  it("rejects -s without a value", () => {
    assert.throws(
      () => execFileSync(process.execPath, [binPath, "-s"], { encoding: "utf8", timeout: 5000 }),
      /-s requires a value/
    );
  });
  ...
  it("rejects unknown options", () => {
    assert.throws(
      () => execFileSync(process.execPath, [binPath, "--bogus"], { encoding: "utf8", timeout: 5000 }),
      /unknown option/
    );
  });
  ...
  it("rejects invalid session ID format", () => {
    assert.throws(
      () => execFileSync(process.execPath, [binPath, "-s", "foo; rm -rf /"], { encoding: "utf8", timeout: 5000 }),
  ...
  it("rejects session ID with spaces", () => {
    assert.throws(
      () => execFileSync(process.execPath, [binPath, "-s", "foo bar"], { encoding: "utf8", timeout: 5000 }),
      /invalid session ID format/
    );
  });
  ...
  it("rejects session ID longer than 128 characters", () => {
    ...
    assert.throws(
      () => execFileSync(process.execPath, [binPath, "-s", longId], { encoding: "utf8", timeout: 5000 }),
      /invalid session ID format/
    );
  });
});
