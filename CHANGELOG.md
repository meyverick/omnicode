...
- Fixed 5-10 second hang on quit. Added `clearTimeout(timer)` in `initTool` after `Promise.race` resolves so stale timers don't keep the event loop alive for up to 30 seconds after graymatter/openspec finish.

## [0.1.2] - 2026-06-22

### Fixed

- Fixed orphaned omniroute processes on quick Ctrl+C. Added `await sleep(100)` between polling iterations in `stopOmnirouteIfIdle` so SIGTERM has time to be processed before cleanup gives up. Restored `process.on("exit")` handler with synchronous fallback.
